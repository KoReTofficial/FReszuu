...
module.exports = (client) => {
  ...
  client.on("message", (msg) => {
    ...
    if (cmd.guildOnly && !guild) {
      return msg.reply("Nie mogę wykonać tego polecenia w DMs!")
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length) {
      if (!guild.me.permissionsIn(channel).has(cmd.botPermissions)) {
        return channel.send(
          `Potrzebuję więcej uprawnień, aby wykonać to polecenie tutaj. Brak uprawnień: \`${cmd.botPermissions.join(
            "`,`",
          )}\``,
        )
      }
    ...
    if (cmd.userPermissions && cmd.userPermissions.length) {
      if (!msg.member.permissionsIn(channel).has(cmd.userPermissions)) {
        return msg.reply(`Przepraszamy brak Ci uprawnień.`)
      }
    }
    ...
    if (cmd.args && !args.length) {
      ...
      if (cmd.usage) {
        reply += `\nwłaściwe byłoby użycie: \`${prefix}${cmd.name} ${cmd.usage}\``
      }
